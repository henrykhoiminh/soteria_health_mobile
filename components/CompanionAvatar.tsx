...
interface CompanionAvatarProps {
  ...
  onPress?: () => void;
  disabled?: boolean;
  // Set to true to force using fallback even if Rive is available
  useFallback?: boolean;
...
export default function CompanionAvatar({
  ...
  onPress,
  disabled = false,
  useFallback = true, // Default to fallback until .riv files are created
  name,
  ...
  useEffect(() => {
    if (riveRef.current && !useFallback && !riveError) {
      try {
  ...
  }, [lightState, useFallback, riveError]);
  ...
  const handlePress = () => {
    ...
    // Trigger press animation for fallback
    Animated.sequence([
      Animated.timing(pulseAnim, {
        toValue: 1.15,
        duration: 100,
  ...
  // Determine opacity based on light state
  const getOpacity = () => {
    switch (lightState) {
      case 'Dormant':
        return 0.3;
  ...
  // Use Rive if available and .riv file exists
  const shouldUseRive = Rive && !useFallback && !riveError;

  const content = shouldUseRive ? (
    <Rive
      ref={riveRef}
    ...
    // Fallback: Animated circle with icon
    <Animated.View
      style={[
