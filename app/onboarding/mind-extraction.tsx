...
import { useOnboarding, MIND_NAME_OPTIONS } from '@/lib/contexts/OnboardingContext';
...
const captions = [
  ...
  { text: "The voice that's always running.", pauseAfter: 1200 },
  { text: "Every traveler's Mind is different.", pauseAfter: 1000 },
  { text: "Yours has been working hard to get you here.", pauseAfter: 1200 },
  { text: 'What would you like to call it?', pauseAfter: 0 },
];

// Screen 7: Mind Extraction
export default function MindExtractionScreen() {
  const router = useRouter();
  const { data, setMindName } = useOnboarding();
  ...
  const handleContinue = () => {
    if (isValid) {
      ...
      router.push('/onboarding/body-extraction');
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.content}>
        {/* Mind orb - prominently displayed */}
        ...
        <Animated.View style={[styles.selectorContainer, { opacity: selectorOpacity }]}>
          {showSelector && (
            <NameSelector
              options={[...MIND_NAME_OPTIONS]}
              selected={data.mindName || null}
