-- =====================================================
-- Updated Hard Reset Function with Milestone Support
-- =====================================================
-- This updates the hard_reset_user_data function to:
-- 1. Support both 'profiles' and 'user_profiles' table names
-- 2. Reset milestone data
-- =====================================================

-- Drop existing function
DROP FUNCTION IF EXISTS hard_reset_user_data(uuid);

-- Create the updated hard reset function
...
BEGIN
  -- Detect which profile table exists
  ...
  ELSE
    RAISE EXCEPTION 'Neither profiles nor user_profiles table found';
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'user_milestones') THEN
  ...
  -- 9. Reset profile journey data (using detected table name)
  ...
  RETURN jsonb_build_object(
    'success', true,
    'profile_table_used', profile_table_name,
